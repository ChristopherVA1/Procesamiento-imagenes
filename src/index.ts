...
import { DefaultSettings } from "../image-processing/src/DefaultSettings.js";
...
function puntos(){
  lienzo1.addEventListener("mousedown", rectangulo);
}
function rectangulo(evt: any): void{
  let positionX:number= evt.offsetX;
  let positionY: number=evt.offsetY;
  arrImg.push(pantalla1.getImageData(0, 0, DefaultSettings.SIZE_WIDTH, DefaultSettings.SIZE_HEIGHT));
  cv++;
...
function bilinealImg(evt: any): void{
  if((arrayx.length)||(arrayy.length)||(cv < 3)){
  alert("Seleccione dos puntos dentro de la imagen");
}else{
  pantalla1.putImageData(arrImg[0], 0, 0);
  var imgsal: ImageType= new ImageType(pantalla1, imgLocal.getImage());
  imgsal.imageArray2DtoData(pantalla2, MathImg.bilineal(imgsal, arrayx, arrayy));
}
//geometrica
document.getElementById("op-puntos").addEventListener('click', puntos, false);
